...
from app.models import Category, Product, OrderProducts, Order
...
async def show_products_by_category_count(query: Update.callback_query):
    """Fetches the count of products per category from the database and shows them."""
    logger.info("Fetching product counts by category from the database")
    ...
    try:
        async with SessionLocal() as session:
            async with session.begin():
                ...
                category_counts = results.all()
                logger.info(f"Found category counts: {category_counts}")

        if not category_counts:
            await query.edit_message_text(text="No hay categorías con productos disponibles.")
            return

        response = "Cantidad de productos por categoría:\n"
        for category_name, count in category_counts:
            response += f"- {category_name}: {count} productos\n"
        ...
        logger.error(f"Error fetching product counts by category: {e}")
        await query.edit_message_text(text="Ocurrió un error al obtener la cantidad de productos por categoría.")


# Función para obtener las ganancias mensuales
async def show_monthly_income(query: Update.callback_query):
    """Fetches the total income for each month from the database and shows it."""
    logger.info("Fetching monthly income from the database")
    ...
    try:
        async with SessionLocal() as session:
            async with session.begin():
                ...
                results = await session.execute(
                    ...
                    .where(Order.status == True)  # Solo considerar órdenes completadas
                    .group_by('month')
                    .order_by('month')
                )
                monthly_income = results.all()
                logger.info(f"Found monthly income: {monthly_income}")

        if not monthly_income:
            await query.edit_message_text(text="No hay ingresos registrados para mostrar.")
            return

        # Formatear la respuesta con los nombres de los meses
        months = [
            "Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio",
            "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"
        ]
        response = "Total de ganancias por mes:\n"
        for month, total_income in monthly_income:
            response += f"{months[int(month) - 1]}: Total de ganancias = ${total_income:.2f}\n"
        ...
        await query.edit_message_text(text="Ocurrió un error al obtener las ganancias mensuales.")
